...
import {
  ...
  deleteTransition,
  assignToProductProgram,
  removeFromProductProgram,
  CreateTransitionInput,
  UpdateTransitionInput,
  UpdateTransitionStatusInput,
  GetTransitionsQuery,
} from './transition-raw.service';
...
export async function assignTransitionToProductProgramHandler(
  request: FastifyRequest<{
    Params: { id: string };
    Body: { productProgramId: string }
  }>,
  reply: FastifyReply
) {
  try {
    ...
    const transition = await assignToProductProgram(id, productProgramId);
...
export async function removeTransitionFromProductProgramHandler(
  request: FastifyRequest<{ Params: { id: string } }>,
  reply: FastifyReply
) {
  try {
    ...
    const transition = await removeFromProductProgram(id);
    return reply.code(200).send({
      success: true,
