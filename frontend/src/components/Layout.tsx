...
export function Layout({ children, pageTitle = "Dashboard" }: LayoutProps) {
  ...
  const hasAdminRole = isEnhancedAuth && enhancedUser && (
    enhancedUser.roles?.includes('admin') ||
    enhancedUser.roles?.includes('super_admin') ||
    enhancedUser.isFirstUser
  );
