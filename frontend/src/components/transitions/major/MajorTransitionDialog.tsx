...
interface MajorTransitionDialogProps {
  contractId: string;
  onTransitionCreated: (transition: any) => void;
  userRole: string;
}

export function MajorTransitionDialog({ contractId, onTransitionCreated, userRole }: MajorTransitionDialogProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    transitionSource: 'STRATEGIC' as const,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/enhanced-transitions/major', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Failed to create major transition');
      }
      ...
      setFormData({
        ...
        status: 'NOT_STARTED',
        requiresContinuousService: true,
        transitionSource: 'STRATEGIC',
        createdBy: 'default-user-id',
      });
    } catch (err) {
      console.error('Failed to create major transition:', err);
      setError(err instanceof Error ? err.message : 'Failed to create major transition');
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Create Major Transition
            <Badge className="bg-red-100 text-red-800">Organization Level</Badge>
          </DialogTitle>
          <p className="text-sm text-muted-foreground">
            Major transitions are organization-level changes like new contracts or organizational realignments.
          </p>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input
              ...
              placeholder="e.g., Contract ABC Implementation"
              required
            />
          </div>
          ...
          <div>
            ...
            <Textarea
              ...
              placeholder="Detailed description of the major organizational change..."
              rows={3}
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="transitionSource">Transition Source</Label>
              <Select value={formData.transitionSource} onValueChange={(value) => setFormData({ ...formData, transitionSource: value as any })}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="CONTRACTUAL">Contract Change</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Names of key personnel involved..."
            />
          </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? 'Creating...' : 'Create Major Transition'}
            </Button>
          </div>
        </form>
      </DialogContent>
